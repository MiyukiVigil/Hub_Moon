...
def main():
    ...
    parser.add_argument("--gui", action="store_true", help="Launch the desktop EQ GUI (needs slint)")

    args = parser.parse_args()

    # Desktop GUI. Lazy-imported so the CLI keeps its "two dependencies, one file"
    # promise -- slint is only pulled in when someone actually asks for --gui.
    if args.gui:
